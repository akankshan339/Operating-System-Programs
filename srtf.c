...
int main()
{
  ...
  int last_executed_process = -1, ganttIndex = 0, ganttProcessOrder[10];
  printf("Enter total number of processes: ");
  scanf("%d", &n);
  for (int i = 0; i < n; i++)
  {
    printf("Enter Process %d Arrival Time and Burst Time: ", i);
  ...
  while (completed != n)
  {
    ...
    else
    {
      ...
      // Update Gantt chart only if the process changes
      if (last_executed_process != ps[min_index].pid)
      {
        ganttProcessOrder[ganttIndex++] = ps[min_index].pid;
        last_executed_process = ps[min_index].pid;
      }
